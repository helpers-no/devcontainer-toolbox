...
> ⚠️ **These commands are for YOUR project repo.**
>
> On the coordination repo (`urb-agents`) they are wrong: the `to:`/`from:`/`state:` labels are the
> routing and the close authority, and `gh issue close` enforces neither. Use
> `ops/bus/fleet-task.sh` there — see **[COORDINATION.md](COORDINATION.md)**.
>
> `gh` itself is not restricted. The restriction is only the fleet bus.
